      ...
      <div className="relative bg-[#1a472a]/30 rounded-2xl overflow-hidden border border-[#b8860b]/10 hover:border-[#b8860b]/30 transition-all duration-500">
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
          {product.isBestseller && (
            <span className="px-3 py-1 bg-[#b8860b] text-[#0d1f14] text-xs font-bold rounded-full">Bestseller</span>
          )}
          {product.isNew && (
            <span className="px-3 py-1 bg-[#f5f0e1] text-[#0d1f14] text-xs font-bold rounded-full">New</span>
          )}
          {discount > 0 && (
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-sm font-semibold text-[#f5f0e1]">{product.rating}</span>
            <span className="text-sm text-[#f5f0e1]/40">({product.reviews})</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-bold text-[#f5f0e1]">₹{product.price}</span>
              {product.originalPrice && (
                <span className="text-sm text-[#f5f0e1]/40 line-through">₹{product.originalPrice}</span>
              )}
            </div>
            <button
            ...
            >
              <ShoppingBag className={`w-5 h-5 ${isAdding ? "text-white" : "text-[#0d1f14]"}`} />
            </button>
          </div>

          {!product.inStock && <p className="text-red-400 text-sm mt-2">Out of Stock</p>}
        </div>
      </div>
    </Link>
  )
